<%@ page language="java" import="java.util.*" pageEncoding="ISO-8859-1"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<%@ taglib uri = "http://java.sun.com/jsp/jstl/functions" prefix = "fn" %>
<link href="wrsis/css/dashboard.css" rel="stylesheet">
<!-- <script src="wrsis/js/dashboard.js"></script> -->
<script src="https://code.highcharts.com/highcharts.js"></script>
<script src="https://code.highcharts.com/modules/exporting.js"></script> 
 <%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>

    <link href="wrsis/css/bootstrap-datetimepicker.min.css" rel="stylesheet">
     <link href="wrsis/css/style.css" rel="stylesheet">
     <style>
     	.suspectedPlot a{color: #dc356f;}
     </style>


<div class="mainpanel">
            <div class="section">
               <section class="content_section">
        <div class="row">
              <div class="col-lg-3 col-md-6 col-sm-12 mrg-btm-3">
                <div class="card border_danger top_card">
                    <div class="card-body p-3">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <div class="me-2">
                                <div class="display-5 font-weight-bold">${distCount} </div>
                                <div class="card-text">Districts</div>
                            </div>
                            <div class="icon-circle">
                                <svg xmlns="http://www.w3.org/2000/svg" class="district" width="174.909" height="137" viewBox="0 0 174.909 137">
                                    <g id="districts" transform="translate(653.044 -11.102)">
                                      <path id="Path_954" data-name="Path 954" d="M-526.187,228.986c2.16-3.118,4.059-6.465,6.54-9.3,4.031-4.612,5.475-10.1,6.349-15.88.2-1.346.3-2.709.507-4.054.453-2.9.762-5.3-3.6-5.448-1.312-.044-2.533-2.822-4.4-5.082,2.284-.474,3.459-1.03,4.548-.894,10.422,1.3,10.208-2.027,14.85,10.1,7.266,18.986,14.752,37.889,22.141,56.829a21.825,21.825,0,0,1,.786,2.25c1.262,4.7-1.077,7.93-5.952,7.955-12.707.066-25.414.029-38.121.03-21.56,0-43.119-.032-64.678.011-14.861.029-29.722.189-44.583.23-4.994.013-9.99-.128-14.982-.26-4.855-.128-7.32-3.367-5.812-8.024,1.4-4.315,3.2-8.5,4.856-12.73q7.987-20.473,16-40.935c.4-1.03.957-2,1.458-3.036,3.581,2.523,4.489,4.856,3.152,8.393-2.57,6.8-5.253,13.553-7.878,20.33-.436,1.125-.814,2.271-1.377,3.855,5.988,0,11.473.014,16.957-.031.372,0,.8-.394,1.1-.7,3.664-3.684,7.352-7.344,10.949-11.093,4.976-5.186,5.788-10.814,2.218-17.1-1.885-3.317.488-4.888,1.963-7.029,5.3,4.144,7.774,12.931,5.8,19.608-2.111,7.154-8.173,11.135-13.787,16.648,8.5,0,16.043.01,23.588-.008,1.7,0,3.445-.387,5.088-.111a10.843,10.843,0,0,0,9.442-2.609c1.944-1.677,3.555-1.577,4.632,1.166.9,2.289,2.146,4.44,3.6,7.376a37.086,37.086,0,0,1-5.855.208,6.014,6.014,0,0,0-6.109,2.735c-3.706,4.916-5.751,10.45-7.014,17.156,16.787-.517,33.141.634,49.557.149,16.369-.484,32.764-.106,50-.106-2.658-6.737-4.894-12.757-7.5-18.614-.382-.859-2.508-1.311-3.832-1.324-10.661-.1-21.323-.053-31.985-.05h-2.685c1.038-5.463,2.177-6.18,7.254-6.068,8.62.192,17.247.123,25.871.13a13.832,13.832,0,0,0,2.457-.555l-11.977-30.869c-1.808,13.011-5.76,23.542-18.115,28.026Zm-57.469,10.63c-9.626,0-19.351-.073-29.071.127-.859.018-2.276,1.738-2.422,2.806-.778,5.692,1.22,10.739,4.43,15.312a4.048,4.048,0,0,0,2.8,1.414c5.092.217,10.194.31,15.289.232a3.305,3.305,0,0,0,2.474-1.684C-587.924,251.938-585.9,245.975-583.656,239.616Zm-34.077,19.6a27.283,27.283,0,0,1-1.434-3.54c-.893-3.813-2.05-7.615-2.413-11.487-.416-4.433-.094-4.432-4.75-4.4-2.034.013-4.069-.193-6.1-.188-6.616.016-6.608.039-8.945,6.073-1.7,4.382-3.42,8.755-5.292,13.542Z" transform="translate(0 -117.631)"/>
                                      <path id="Path_955" data-name="Path 955" d="M-418.956,178.457c.418-14.726,11.729-25.724,27.11-26.061,12.845-.281,25.149,11.47,25.657,24.786.262,6.861-2.066,13.245-5.124,19.225-5.626,11-11.513,21.876-17.465,32.709-2.33,4.242-4.724,4.185-7.211.125-8.309-13.561-16.259-27.315-21.24-42.529A73.689,73.689,0,0,1-418.956,178.457Zm26.568,44.833c7.422-13.468,14.891-25.8,19.164-39.68a15.439,15.439,0,0,0,.641-6.031c-.94-8.695-5.565-14.795-13.811-17.733-7.451-2.654-14.258-.764-19.757,4.643-5.995,5.894-8.188,13.255-4.842,21.2,4,9.492,8.64,18.718,13.13,28C-396.4,216.708-394.54,219.542-392.388,223.29Z" transform="translate(-155.392 -93.791)"/>
                                      <path id="Path_956" data-name="Path 956" d="M-598.331,11.1a26.094,26.094,0,0,1,24.838,33.453c-3.684,12.517-10.2,23.731-16.558,35-1.729,3.062-3.459,6.137-5.419,9.052-1.819,2.7-4.315,2.726-5.982,0-8.464-13.859-16.782-27.818-21.824-43.385A26.025,26.025,0,0,1-598.331,11.1Zm-.506,69.852c.557-.276.84-.32.933-.476,7.247-12.32,14.385-24.7,18.7-38.441a17.078,17.078,0,0,0-.784-11.972c-4.257-10.141-15.172-15.272-24.812-11.689-10.746,3.994-16.4,14.9-12.245,25.572,3.772,9.683,8.674,18.932,13.209,28.309C-602.381,75.268-600.507,78.076-598.837,80.954Z" transform="translate(-18.896 0)"/>
                                      <path id="Path_957" data-name="Path 957" d="M-499.714,194.888c.962-3.89,2.369-6.421,6.337-6.4,6.3.033,12.6.009,19.325.009-.834,3.424-.976,6.389-4.909,6.391C-485.709,194.891-492.457,194.888-499.714,194.888Z" transform="translate(-101.783 -117.752)"/>
                                      <path id="Path_958" data-name="Path 958" d="M-362.077,213.868a11.158,11.158,0,0,1,11.35,10.752,11.577,11.577,0,0,1-11.645,11.7,11.162,11.162,0,0,1-10.808-11.3A10.742,10.742,0,0,1-362.077,213.868Zm5.038,11.264a5.3,5.3,0,0,0-4.873-5.182,4.783,4.783,0,0,0-4.882,4.62c-.181,3.139,1.47,5.028,4.5,5.144A4.8,4.8,0,0,0-357.039,225.132Z" transform="translate(-185.779 -134.6)"/>
                                      <path id="Path_959" data-name="Path 959" d="M-580,83.08c0-6.615,4.484-11.124,11.083-11.136,6.583-.013,11.255,4.718,11.278,11.42A11.322,11.322,0,0,1-568.51,94.307C-575.167,94.3-580,89.58-580,83.08Zm16.139.069a5.263,5.263,0,0,0-4.928-5.118,5.307,5.307,0,0,0-5.125,4.935,5.265,5.265,0,0,0,4.931,5.114A5.279,5.279,0,0,0-563.86,83.149Z" transform="translate(-48.489 -40.388)"/>
                                    </g>
                                  </svg>
                            </div>
                        </div>
                        <div class="card-text">
                            <div class="d-inline-flex align-items-center">
                              <!--   <i class="fa fa-home text-danger" aria-hidden="true"></i> -->
                                <!-- <i class="fa fa- text-success"></i> -->
                                <div class="caption text-danger fw-500 ml-1 mr-1 suspectedPlot"><a href="viewSuspectedPlot"> ${totalSurveyCount}</a> Lands</div>
                                <div class="caption">Suspected</div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-3 col-md-6 col-sm-12 mrg-btm-3">
                <div class="card border_primary top_card">
                    <div class="card-body p-3">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <div class="me-2">
                                <div class="display-5 font-weight-bold">${totalSurveyorsCount}</div>
                                <div class="card-text">Total Surveyors</div>
                            </div>
                            <div class="icon-circle">
                                <svg xmlns="http://www.w3.org/2000/svg" class="surveyor" width="140.032" height="137" viewBox="0 0 140.032 137">
                                    <g id="total-surveyors" transform="translate(1410.512 2)">
                                      <path id="Path_960" data-name="Path 960" d="M-1293.316,236.093V178.464h6.165v43.555c5.035-.447,9.867-5.663,10.138-11.131.137-2.779.117-5.622.1-8.371-.007-1.037-.015-2.074-.014-3.111v-.8c.007-10.7.014-21.76,0-32.639-.012-7.91-5.682-13.7-13.481-13.773-5.643-.051-11.326-.077-16.89-.077-5.749,0-11.539.028-17.208.083a13.034,13.034,0,0,0-12.969,10.49l15.285,0q5.357,0,10.714.008c1.923,0,3.12.418,3.767,1.3s.676,2.147.094,3.971c-2.678,8.4-5.366,16.8-8.081,25.287l-2.913,9.1c4.968,2.246,4.605,6.695,3.708,10.362-.192.784-.332,1.607-.467,2.4-.107.629-.218,1.281-.355,1.921-.8,3.746-3.064,6.085-6.539,6.765-1.04.2-1.149.462-1.131,1.4.046,2.418.038,4.784.029,7.288q-.005,1.467-.007,2.955v.632h-6.417v-.632q0-1.563-.009-3.116c-.011-2.526-.022-5.137.036-7.7a3.015,3.015,0,0,0-1.343-2.763,15.047,15.047,0,0,1-2.662-3.223c-.375-.552-.764-1.123-1.171-1.64-.062-.079-.127-.164-.193-.251-.217-.285-.67-.879-.876-.929-1.373-.055-2.868-.081-4.838-.081q-1.22,0-2.468.008-.984,0-1.992.008v20.32h-6.355V215.755l-1.165.005c-.544,0-1.07.006-1.588.006-.752,0-1.37-.006-1.946-.02-1.629-.038-2.708-.446-3.3-1.248s-.66-1.961-.209-3.538c.2-.712.414-1.422.64-2.188l.1-.328h-30.67v27.618h-6.338V208.449c-.439-.032-.869-.057-1.289-.081a26.727,26.727,0,0,1-3.687-.384,11.316,11.316,0,0,1-9.292-11.3c-.009-3.072.021-6.195.05-9.215.058-6.123.119-12.455-.137-18.661-.457-11.109,8.128-17.979,16.3-19.659a27.232,27.232,0,0,1,5.4-.507q3.16-.014,6.321-.012,3.227,0,6.453.006t6.452.006c4.722,0,8.355-.016,11.78-.051h.257c6.681,0,11.646,2.657,15.179,8.122a19.92,19.92,0,0,1,13.761-10.431,21.575,21.575,0,0,1,4.3-.408l4.449-.013c5.139-.015,10.453-.031,15.68-.031,5.1,0,9.606.015,13.767.046a20.165,20.165,0,0,1,13.034,4.608,19.753,19.753,0,0,1,6.679,11.947,20.321,20.321,0,0,1,.2,3.475v.161c.012,13.842.01,27.345.007,42.3,0,10.72-5.764,17.959-16.224,20.384a2.465,2.465,0,0,0-.332.126l-.022.009v7.213Zm-35.757-22.6c0,.059.007.131.01.21a5.062,5.062,0,0,0,.073.777c.379,1.891,2.072,3.066,4.42,3.067h.114a3.1,3.1,0,0,0,2.2-1.381,33.738,33.738,0,0,0,1.215-5.96c.05-.348.1-.7.15-1.041a1.191,1.191,0,0,0-.337-.732,7.034,7.034,0,0,0-3.144-.834C-1327.165,207.6-1328.965,209.858-1329.074,213.495Zm-6-3.814c1.3-4.874,4.293-7.637,8.894-8.217a1.646,1.646,0,0,0,1.7-1.424c2.087-6.666,4.232-13.436,6.306-19.984l2.209-6.976c.31-.981.617-1.963.956-3.048l.262-.84h-1.6c-1.963,0-3.9,0-5.832.009s-3.87.008-5.805.008c-4.021,0-7.127-.019-10.071-.061h-.088c-1.306,0-1.756.36-2.19,1.751-2.807,8.989-5.71,18.112-8.516,26.934q-1.711,5.379-3.42,10.758c-.109.343-.191.707-.275,1.09Zm-54.164-54.651a16.09,16.09,0,0,0-10.465,3.6c-2.833,2.315-4.218,4.869-4.234,7.807-.054,9.951-.057,20.231-.007,30.553a4.8,4.8,0,0,0,4.686,4.822c6.07.034,12.522.051,19.724.051,7.62,0,15.239-.019,21.687-.037a1.6,1.6,0,0,0,.949-.428c1.067-3.139,2.061-6.293,3.113-9.631q.323-1.024.651-2.062h-1.639l-9.067-.009q-13.186-.012-26.372-.028c-3.566-.005-4.655-1.107-4.659-4.715-.007-5.623-.007-10.767,0-15.725,0-1.808.332-4.218,3.148-4.278h.1c3.2,0,3.21,3.3,3.212,4.379q.006,2.8,0,5.6,0,3.288.006,6.577c0,.456.043.924.083,1.377l0,.037h32.252l-.545-.513c-1.121-1.054-2.1-1.975-3.073-2.9-2.191-2.091-1.5-3.837-.536-4.934a2.888,2.888,0,0,1,2.178-1.08,4.267,4.267,0,0,1,2.943,1.528c1.278,1.3,2.525,2.654,3.845,4.085q.561.608,1.148,1.243l3.458-10.663a11.117,11.117,0,0,0-1.476-10.081,11.154,11.154,0,0,0-9-4.571q-6.58,0-13.151,0h-18.97Z" transform="translate(0 -101.093)"/>
                                      <path id="Path_961" data-name="Path 961" d="M-1339.995,44.964a22.455,22.455,0,0,1-22.336-22.383,22.124,22.124,0,0,1,6.512-15.767A22.368,22.368,0,0,1-1339.879.227h.072a22.236,22.236,0,0,1,15.778,6.536,22.209,22.209,0,0,1,6.507,15.782,22.244,22.244,0,0,1-6.55,15.849,22.26,22.26,0,0,1-15.853,6.571Zm.094-38.431a16,16,0,0,0-15.957,15.973,15.969,15.969,0,0,0,15.914,15.987,16.012,16.012,0,0,0,16.063-15.887,16.248,16.248,0,0,0-15.963-16.074Z" transform="translate(-32.95 -1.523)"/>
                                      <path id="Path_962" data-name="Path 962" d="M-1131.355,41.152a21.292,21.292,0,0,1-15.176-6.31,21.467,21.467,0,0,1-6.3-15.325A21.569,21.569,0,0,1-1131.351-2h.2a21.636,21.636,0,0,1,21.369,21.633,21.61,21.61,0,0,1-21.487,21.518Zm.011-36.689a15.086,15.086,0,0,0-10.709,4.45,14.96,14.96,0,0,0-4.453,10.668,15.174,15.174,0,0,0,15.173,15.12,14.973,14.973,0,0,0,10.679-4.423,15.053,15.053,0,0,0,4.406-10.709,15.12,15.12,0,0,0-15.09-15.106V3.83Z" transform="translate(-176.219)"/>
                                    </g>
                                  </svg>
                            </div>
                        </div>
                        <div class="card-text">
                            <div class="d-inline-flex align-items-center">
                                <!-- <i class="fa fa-user text-primary" aria-hidden="true"></i> -->
                                <!-- <i class="fa fa- text-success"></i> -->
                                <div class="caption text-primary fw-500 ml-1 mr-1">${newSurveyorsCount} Surveyors</div>
                                <div class="caption">Newly Registered</div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
                <div class="col-lg-3 col-md-6 col-sm-12 mrg-btm-3">
                <div class="card border_success top_card">
                    <div class="card-body p-3">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <div class="me-2">
                                <div class="display-5 font-weight-bold">${totalCompletedSurvey}</div>
                                <div class="card-text">Survey Completed</div>
                            </div>
                            <div class="icon-circle">
                                <svg xmlns="http://www.w3.org/2000/svg" class="survey_complete" width="136.97" height="137" viewBox="0 0 136.97 137">
                                    <g id="survey-completed" transform="translate(1997.138 4)">
                                      <path id="Path_963" data-name="Path 963" d="M-1986.317,133l.052-.674c.007-.089.015-.178.024-.266.016-.169.032-.328.032-.477V110.627c0-29.782,0-60.579.022-90.868,0-.686-.073-.719-.656-.766a11.018,11.018,0,0,1-7.573-4.029,13.854,13.854,0,0,1-2.7-5.966l-.019-.076V6l.115-.163c.018-.026.038-.051.059-.077,1-5.841,5.974-9.756,12.429-9.756h.155c5.645.055,11.773.081,19.286.081q6.511,0,13.021-.017t13.024-.017l21.965,0q10.3,0,20.588-.009a12.014,12.014,0,0,1,7.7,2.407c3.074,2.4,4.566,5.712,4.561,10.128-.026,22.027-.023,44.424-.02,66.083l0,22.1h1.986q2.012,0,4.024-.008t4.027-.008c2.883,0,5.1.018,7.2.058a6.886,6.886,0,0,1,5.442,2.825,7.139,7.139,0,0,1,1.12,6.259c-.942,3.276-3.654,5.169-7.441,5.193q-3.137.02-6.275.017l-7.3-.005h-1.81c-.319,0-.644.022-1.02.046h-.019v8.753h-8.7V133Zm31.094-5.648c17.868,0,37,0,55.872.023.539,0,.665-.085.691-.111s.109-.15.1-.659c-.041-3.481-.035-7.019-.029-10.441q0-2.005,0-4.01a6.463,6.463,0,0,0-.042-1.021,6.813,6.813,0,0,0-1.087-.044h-17.81c-12.251,0-26.5,0-40.643-.018a9.589,9.589,0,0,1-2.913-.439c-4.632-1.505-9.327-3.074-13.868-4.591l-3.086-1.031c-.281-.094-.549-.207-.859-.339-.157-.066-.328-.139-.522-.219l-1.084-.442v23.342Zm65.4-13.14q0-.212,0-.419c.008-.858.015-1.669-.033-2.46a.74.74,0,0,0-.142-.2c-.47-.027-.98-.039-1.633-.039-.313,0-.627,0-.943.005l-.348,0v3.112Zm19.594-8.711c.434.007.859.013,1.279.013.627,0,1.138-.015,1.61-.046a1.477,1.477,0,0,0,1.517-1.5,1.382,1.382,0,0,0-1.389-1.55c-.5-.038-1.036-.055-1.695-.055-.443,0-.888.008-1.337.015l-.77.012v3.1Zm-6.565-.063v-2.99h-2.954v2.99Zm-8.738-.019v-2.99h-70.861v2.99Zm-78.334-.016v-2.974l-4.471,1.489Zm-16.633-1.751,1.123-.374q2.337-.778,4.635-1.546c4.563-1.524,8.873-2.963,13.28-4.392a11.287,11.287,0,0,1,3.415-.56c12.994-.021,25.809-.023,37.721-.023h21.625V19.29h-81.8Zm90.678-6.963V83.771q0-37.6-.013-75.194a10.043,10.043,0,0,0-.286-2.586,5.815,5.815,0,0,0-5.713-4.312,6.679,6.679,0,0,0-1.028.081,6.054,6.054,0,0,0-4.892,5.948,6,6,0,0,0,5.5,5.8c.268.022.557.033.911.033.265,0,.535-.006.812-.012.3-.006.612-.013.934-.013h.626v83.17ZM-1920.2,1.672q-32.124,0-64.247,0c-.628,0-1.176.018-1.675.055a5.78,5.78,0,0,0-4.294,2.458,5.834,5.834,0,0,0-.88,4.917c.773,2.823,3.055,4.442,6.26,4.442q18.315,0,36.63,0l42.5,0c.081,0,.163,0,.247-.005a11.434,11.434,0,0,1-.013-11.87Z"/>
                                      <rect id="Rectangle_219" data-name="Rectangle 219" width="46.965" height="5.493" transform="translate(-1948.81 48.646)"/>
                                      <rect id="Rectangle_220" data-name="Rectangle 220" width="46.979" height="5.381" transform="translate(-1948.819 70.59)"/>
                                      <rect id="Rectangle_221" data-name="Rectangle 221" width="38.272" height="5.353" transform="translate(-1948.867 35.558)"/>
                                      <rect id="Rectangle_222" data-name="Rectangle 222" width="33.887" height="5.46" transform="translate(-1935.771 57.408)"/>
                                      <rect id="Rectangle_223" data-name="Rectangle 223" width="33.909" height="5.378" transform="translate(-1948.869 26.802)"/>
                                      <rect id="Rectangle_224" data-name="Rectangle 224" width="33.899" height="5.352" transform="translate(-1948.867 79.346)"/>
                                      <path id="Path_964" data-name="Path 964" d="M-1932.444,98.251l4.036-3.705,2.812,2.633,11.519-9.569,3.744,4.434-15.494,12.922Z" transform="translate(-44.429 -62.914)"/>
                                      <path id="Path_965" data-name="Path 965" d="M-1932.569,168.278l4.141-3.52,2.5,2.785,11.844-9.86,3.586,4.305-15.481,12.9Z" transform="translate(-44.343 -111.039)"/>
                                      <path id="Path_966" data-name="Path 966" d="M-1932.557,238.234l4.077-4.016,2.654,3.159,11.747-9.76,3.6,4.282-15.478,12.91Z" transform="translate(-44.352 -159.066)"/>
                                      <rect id="Rectangle_225" data-name="Rectangle 225" width="9.864" height="5.425" transform="translate(-1948.852 57.432)"/>
                                      <rect id="Rectangle_226" data-name="Rectangle 226" width="9.775" height="5.411" transform="translate(-1911.651 26.796)"/>
                                      <rect id="Rectangle_227" data-name="Rectangle 227" width="9.811" height="5.347" transform="translate(-1911.646 79.338)"/>
                                    </g>
                                  </svg>
                            </div>
                        </div>
                        <div class="card-text">
                            <div class="d-inline-flex align-items-center">
                               <!--  <i class="fa fa-file text-success" aria-hidden="true"></i> -->
                                <!-- <i class="fa fa- text-success"></i> -->
                                <div class="caption text-success fw-500 ml-1 mr-1">${approveSurveyCount} Survey</div>
                                <div class="caption">Approved By DRCS</div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
           <div class="col-lg-3 col-md-6 col-sm-12 mrg-btm-3">
                <div class="card border_warning top_card">
                    <div class="card-body p-3">
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <div class="me-2">
                                <div class="display-5 font-weight-bold">${totalPendingSurveyCount}</div>
                                <div class="card-text">Survey Pending</div>
                            </div>
                            <div class="icon-circle">
                                <svg xmlns="http://www.w3.org/2000/svg" class="pending" width="113.983" height="137" viewBox="0 0 113.983 137">
                                    <g id="survey-pending" transform="translate(2390.55 -5.952)">
                                      <path id="Path_967" data-name="Path 967" d="M-2376.482,142.952a14.241,14.241,0,0,1-10.051-4.142,14.279,14.279,0,0,1-4.017-10.429c.086-12.7.067-25.612.049-38.1v-.045q-.011-7.958-.015-15.917V58.772q0-19.252.007-38.5c.005-6.86,3.817-12.152,9.946-13.811a15.548,15.548,0,0,1,4.019-.481c14.685-.016,30.766-.024,50.607-.024q15.328,0,30.651,0c8.149,0,13.9,5.6,13.981,13.613.115,11.07.079,22.331.046,33.221-.008,2.424-.015,4.822-.021,7.219-.006,2.373-1.479,3.968-3.665,3.968h-.05c-2.151-.023-3.6-1.674-3.6-4.107-.015-9.135-.014-18.426-.012-27.41,0,0,0-7.88,0-11.735,0-5.313-2.285-7.58-7.639-7.58h-79.458c-5.09,0-7.46,2.36-7.461,7.425q0,53.889,0,107.778c0,5,2.35,7.334,7.394,7.335l17.86,0q7.616,0,15.232.008a3.67,3.67,0,0,1,3.761,3.264,3.62,3.62,0,0,1-2.646,3.772,6.985,6.985,0,0,1-1.748.141h-.806c-3.18,0-6.486-.018-9.4-.035s-6.234-.035-9.417-.035c-5.109,0-9.293.048-13.171.15C-2376.236,142.95-2376.359,142.952-2376.482,142.952Z"/>
                                      <path id="Path_968" data-name="Path 968" d="M-2162.528,283.862q-8.247,0-16.492-.007a3.869,3.869,0,0,1-3.141-1.263,2.536,2.536,0,0,1-.355-2.289,2.5,2.5,0,0,1,2.128-1.992c.548-.1,1.12-.145,1.783-.2.332-.027.676-.055,1.039-.091l1.182-.118v-1.188c0-.981.013-1.958.026-2.934.03-2.224.062-4.525-.072-6.8a6.827,6.827,0,0,1,2.452-6.009c2.295-2.065,4.41-4.215,6.649-6.492.93-.945,1.891-1.923,2.866-2.892l.933-.929-.931-.93q-1.526-1.525-3.017-3.029c-2.51-2.524-4.867-4.9-7.325-7.27a4.83,4.83,0,0,1-1.6-3.824c.047-2.7.038-5.453.028-8.116,0-1.155-.008-2.294-.008-3.441v-1.185l-1.179-.121c-.337-.034-.662-.061-.979-.087a16.481,16.481,0,0,1-1.744-.2,2.819,2.819,0,0,1-2.375-2.717,2.846,2.846,0,0,1,2.479-2.77,4.277,4.277,0,0,1,.745-.071l7.534,0q7.108,0,14.215,0,8.777,0,17.554.015c2.039,0,3.217,1.019,3.232,2.785.014,1.636-.976,2.655-2.789,2.868-.408.048-.844.061-1.349.075-.281.008-.559.016-.857.029l-1.254.056v1.255c0,1.04-.018,2.1-.035,3.124-.04,2.4-.081,4.854.093,7.278a7.293,7.293,0,0,1-2.715,6.527c-2.156,1.922-4.123,3.935-6.2,6.067-.876.9-1.769,1.812-2.669,2.71l-.93.929.93.929q1.425,1.424,2.822,2.839l.028.029c2.231,2.253,4.535,4.579,6.864,6.785a5.383,5.383,0,0,1,1.856,4.487c-.1,2.588-.08,5.22-.058,7.764v.04c.009,1.082.018,2.168.018,3.26v1.225l1.222.084c.333.023.65.038.958.053a15.25,15.25,0,0,1,1.637.134,2.633,2.633,0,0,1,2.432,2.742,2.68,2.68,0,0,1-2.406,2.793,6.836,6.836,0,0,1-1.228.08h-2.428Q-2152.745,283.861-2162.528,283.862Zm.036-26.413c-2.545,2.558-4.95,4.975-7.39,7.458a2.911,2.911,0,0,0-.812,1.9c-.025,2.548-.023,5.039-.021,7.677v3.543h22.01v-3.5c0-2.712,0-5.273-.013-7.9a2.708,2.708,0,0,0-.664-1.8c-2.486-2.516-4.936-4.964-7.529-7.555l-2.708-2.707Zm-8.238-30.964c0,2.653,0,5.158.019,7.7a3.128,3.128,0,0,0,.775,2.074c2.418,2.475,4.829,4.9,7.381,7.462l3.042,3.059.931-.959c.631-.65,1.222-1.3,1.793-1.929a30.778,30.778,0,0,1,3.692-3.643,10.917,10.917,0,0,0,4.422-10.81,21.828,21.828,0,0,1-.072-3.645c.019-.557.041-1.186.041-1.794v-1.313h-22.023Z" transform="translate(-139.667 -141.717)"/>
                                      <path id="Path_969" data-name="Path 969" d="M-2329.163,88.3c-2.354,0-3.61-.639-4.335-2.206a3.416,3.416,0,0,1,.171-3.306,4.09,4.09,0,0,1,3.459-1.813c1.213-.019,2.518-.028,4.107-.028q1.586,0,3.172.008t3.182.008l39.357,0q7.017,0,14.034.008c2.533,0,4.242,1.461,4.252,3.628a3.5,3.5,0,0,1-2.941,3.553,9.552,9.552,0,0,1-1.717.133q-10.9,0-21.808,0h-1.739q-19.591,0-39.182.014Z" transform="translate(-38.098 -50.385)"/>
                                      <path id="Path_970" data-name="Path 970" d="M-2266.009,169.934q-5.085-.018-10.17-.015l-11.777,0h-22.486q-9.226,0-18.45-.008h-.463a7.762,7.762,0,0,1-1.818-.145,3.571,3.571,0,0,1-2.609-3.8,3.657,3.657,0,0,1,3.445-3.261c.079,0,.158-.006.236-.006l.342,0h28.451q17.789,0,35.577-.008a3.581,3.581,0,0,1,3.671,2.178,3.329,3.329,0,0,1-.662,3.909,3.9,3.9,0,0,1-3.247,1.153Z" transform="translate(-38.128 -105.304)"/>
                                      <path id="Path_971" data-name="Path 971" d="M-2325.651,333.121q-1.939,0-3.877-.01c-2.522-.015-4.224-1.481-4.235-3.647a3.379,3.379,0,0,1,1.125-2.633,4.569,4.569,0,0,1,3.062-1.029c3.17-.017,6.493-.025,9.878-.025,3.237,0,6.677.008,10.225.024,2.553.011,4.206,1.435,4.211,3.627.006,2.3-1.612,3.674-4.326,3.685q-1.945.008-3.891.007l-6.08,0Z" transform="translate(-38.151 -214.868)"/>
                                      <path id="Path_972" data-name="Path 972" d="M-2319.562,251.622l-6.158,0q-1.987,0-3.975-.008c-2.386-.011-4.059-1.5-4.069-3.612a3.4,3.4,0,0,1,.948-2.477,4.285,4.285,0,0,1,3.076-1.122q5.109-.013,10.218-.013t10.219.011c2.45.006,4.034,1.406,4.037,3.567,0,2.2-1.567,3.632-4,3.645q-2.027.01-4.056.009Z" transform="translate(-38.151 -160.193)"/>
                                    </g>
                                  </svg>
                            </div>
                        </div>
                        <div class="card-text">
                            <div class="d-inline-flex align-items-center">
                               <!--  <i class="fa fa-clone text-warning" aria-hidden="true"></i> -->
                                <!-- <i class="fa fa- text-success"></i> -->
                                <div class="caption text-warning fw-500 ml-1 mr-1">${distYetToComplete} Districts</div>
                                <div class="caption">are Yet to Complete</div>                               
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-6 col-md-6 mrg-btm-3">
                <div class="card">
                    <div class="card-header">
                        <p>Survey Status</p>
                    </div>
                    <div class="card-body">
                        <div class="chart_box">
                            <div class="row">
                                <div class="col-lg-8">
                                    <div id="survey_status"></div>
                                </div>
                                <div class="col-lg-4">
                                    <div class="data_box">
                                        <p class="total_count mb-0">${totalSurveyCount}</p>
                                        <p class="total_text">Total Reported Survey</p>
                                        <p class="completed_count mb-0">${totalCompletedSurvey}</p>
                                        <p class="total_text">Completed Survey</p>
                                        <p class="pending_count mb-0">${totalPendingSurveyCount}</p>
                                        <p class="total_text">Pending Survey</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-lg-6 col-md-6 mrg-btm-3">
                <div class="card">
                    <div class="card-header">
                        <p>District Wise Survey</p>
                    </div>
                    <div class="scrollbar_table" id="style-3">
                    <div class="force-overflow">
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table survey_district">
                                <thead>
                                    <tr>
                                        <th class="text-center">Sl#</th>
                                        <th>District</th>
                                        <th class="text-right">Total Surveyors</th>
                                        <th class="text-right">Total Survey</th>
                                        <th class="text-right">Completed Survey</th>
                                        <th class="text-right">Pending Survey</th>
                                        
                                    </tr>
                                </thead>
                                <tbody>
                                <c:forEach var="listValue" items="${districtWiseSurveyList}"
									varStatus="myIndex">
                                   <tr>
                                        <td class="text-center" width="40"><c:out value="${(myIndex.index)+1}" /></td>
                                        <td>${listValue.distName}</td>
                                        <td class="text-right font-weight-bold" width="300">${listValue.totalSurveyors}</td>
                                        <td class="text-right font-weight-bold text-info" width="300">${listValue.totalSurvey}</td>
                                        <td class="text-right font-weight-bold text-success" width="300">${listValue.completedSurvey}</td>
                                        <td class="text-right font-weight-bold text-warning" width="300">${listValue.pendingSurvey}</td>
                                       
                                    </tr>
                       </c:forEach>
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
                </div>
                </div>
            </div>
        
        </div>
        </div>
        <script src="wrsis/js/moment.min.js"></script>
        <script src="wrsis/js/bootstrap-datetimepicker.min.js"></script>
       <script src="wrsis/js/new_highchart.js"></script>
    </section>

    <script>
        $(document).ready(function () {
            // date funtion
    /*         $('.datepicker').datetimepicker({
                format: 'LT',
                format: 'DD-MM-YYYY',
                daysOfWeekDisabled: [0, 6],
            }); */
            
            $(".survey_filter").click(function(){
                $(".fltrReport").slideToggle();
                $(this).find('i').toggleClass('fa-filter fa-times')
            });
        });
    </script>
                       <script>
            console.log(parseFloat("${surveyCompletedPerc}"))
            console.log(parseFloat("${surveyPendingPerc}"))
            Highcharts.chart('survey_status', {
            	  chart: {
            	    plotBackgroundColor: null,
            	    plotBorderWidth: null,
            	    plotShadow: false,
            	    type: 'pie'
            	  },
            	  colors: ['#18bd77', '#ffbc5d'],
            	  title: {
            	    text: ''
            	  },
            	  tooltip: {
            	    pointFormat: '{series.name}:<b>{point.percentage:.1f}%</b>'
            	  },
            	  accessibility: {
            	    point: {
            	      valueSuffix: '%'
            	    }
            	  },
            	  plotOptions: {
            	    pie: {
            	      allowPointSelect: true,
            	      cursor: 'pointer',
            	    //   size: 300
            	    //   dataLabels: {
            	    //     enabled: true,
            	        
            	    //     // shadow: "#ff0000",
            	    //   }
            	    }
            	  },
            	    legend: {
            	                align: 'right',
            	                verticalAlign: 'middle',
            	                layout: 'vertical',
            	                x: -50,
            	                y: 150,
            	                itemStyle: {
            	                    color: 'grey',
            	                    fontWeight: 'normal',
            	                },
            	                symbolRadius: 4
            	            },
            	  credits: {
            	                enabled: false
            	            },
            	  series: [{
            	    name: 'Survey',
            	    colorByPoint: true,
            	    data: [{
            	      name: 'Completed',
            	      y: parseFloat("${surveyCompletedPerc}"),
            	      sliced: true,
            	      selected: true
            	    }, {
            	      name: 'Pending',
            	      y: parseFloat("${surveyPendingPerc}")
            	    }],
            	    dataLabels: {
            	                    enabled: true,
            	                    rotation: 0,
            	                    color: '#000',
            	                    align: 'center',
            	                    format: '<b>{point.name}</b>: <br/> {point.percentage:.1f} %',
            	                    distance: -55,
            	                    // y: -10, // 10 pixels down from the top
            	                    style: {
            	                        fontSize: '12px',
            	                        fontFamily: 'helvetica, arial, sans-serif',
            	                        textShadow: false,
            	                        fontWeight: 'bold',
            	                        

            	                    }
            	                }
            	  }]
            	});
            </script>
            </div>
            </div>

               <hr>

                                 